...
def setup_agent(state: AgentState) -> Dict[str, Any]:
    ...
    config = Config.interactive(
        ...
        CLOCK_PORT="clk", CLOCK_NET="clk", CLOCK_PERIOD=10 , # 10 ns = 100 MHz
...
# ... (All other physical step agents from the original code remain the same)
...
def check_simulation(state: AgentState) -> str:
    if state["simulation_passed"]:
        ...
        if state.get("update_attempt", 0) > 0:
            st.warning("Looping back to Verilog Corrector for another attempt.")
            return "fix_verilog" # Loop back to corrector if sim fails after an update
        else:
            st.error("This flow assumes correct initial code. Halting.")
            return "end"
...
# Signoff Flow
signoff_chain = [
    "render_routing", "fill_insertion", "rcx", "sta", "stream_out",
    "render_gds", "drc", "spice_extraction", "lvs"
]
...
st.title("🤖 LLM for Chip Design Automation (v5 - With File Saver)")
st.write("This application verifies the design first, then runs the PnR flow. If the floorplan is too large, it uses an LLM to optimize the design, saves the new files, and re-verifies.")
...
st.graphviz_chart(
    ...
    digraph {
        ...
        subgraph cluster_correction {
            label = "Optimization Loop (if needed)";
            style=filled;
            color="#ffe8e8";
            verilog_corrector [label="A. Verilog Corrector (LLM)", fillcolor="#f6d5f7"];
            code_decomposer [label="B. Code Decomposer", fillcolor="#f6d5f7"];
            tb_corrector [label="C. Testbench Corrector (LLM)", fillcolor="#f6d5f7"];
            file_saver [label="D. Save Updated Files", fillcolor="#d4edda"];
        }
        ...
        subgraph cluster_signoff {
            ...
            style=filled;
            color=lightgrey;
            signoff_flow [label="7. Signoff Flow"];
        }
        ...
        icarus_simulation -> verilog_corrector [label="Sim Fail after update", style=dashed, color=red];
        icarus_simulation -> end_node [label="Initial Sim Fail", style=dashed, color=red];

        setup -> synthesis -> floorplan;
        floorplan -> pnr_flow [label="Size OK"];
        pnr_flow -> signoff_flow;
        signoff_flow -> end_node;

        // Correction Loop
        floorplan -> verilog_corrector [label="Size Too Big", style=dashed, color=red, constraint=false];
        verilog_corrector -> code_decomposer [style=dashed, color=orange];
        code_decomposer -> tb_corrector [style=dashed, color=orange];
        tb_corrector -> file_saver [style=dashed, color=orange];
        file_saver -> icarus_simulation [label="Re-Verify", style=dashed, color=green];
    }
    """
)
...
if uploaded_files:
    verilog_file_names = [f.name for f in uploaded_files if f.name.endswith(".v") and "tb" not in f.name.lower()]
            ...
            if not llm:
                ...
                try:
                    with st.spinner("🚀 Agents at work... This flow involves LLM calls and a full PnR, it will take several minutes."):
                        ...
                        # Use a high recursion limit for the feedback loop
