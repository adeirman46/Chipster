...
#     1.  **Update Instantiation:** Ensure the testbench correctly instantiates and connects to the design under test.
...
#     # CORRECTED: Sanitize the string by removing non-ASCII characters before searching.
#     ascii_output = simulation_output.encode('ascii', 'ignore').decode('ascii')
    
#     if re.search(r'\b(PASSED|SUCCESS)\b', ascii_output, re.IGNORECASE):
#         st.success("✅ Verification PASSED: Success keyword found in simulation output.")
#         return {"simulation_verified": True}
#     else:
#         st.error("❌ Verification FAILED: Success keyword not found in simulation output.")
#         st.write("Sanitized output that was checked:")
#         st.code(ascii_output)
...
#     return testbench_corrector_agent(state)
...
#         return "verify_simulation_output"
#     else:
...
# workflow.add_edge("file_saver", "icarus_simulation")
...
# workflow.add_edge("sta_correction", "setup")
...
# workflow.add_edge("pin_reduction_decomposer", "design_name_updater")
# workflow.add_edge("pin_reduction_testbench", "pin_reduction_saver")
# workflow.add_edge("pin_reduction_saver", "icarus_simulation")
...
#     simulation_verifier -> setup [label="Verify OK", color=darkgreen, fontcolor=darkgreen];
...
#     sta_correction -> setup [style=dashed, color=blue, arrowhead=normal, label="Re-run PnR"];
...
#     pin_reduction_decomposer -> design_name_updater [style=dashed, color="#E65100"];
...
def design_name_updater_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Find all defined modules (from filenames)
    defined_modules = {Path(f).stem for f in decomposed_files.keys()}
    ...
    for content in decomposed_files.values():
        ...
        for module_name in matches:
            ...
            if module_name not in ["module", "input", "output", "wire", "reg", "always", "assign"]:
                instantiated_modules.add(module_name)
...
def file_saver_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Create a new versioned directory for this attempt's files
    save_dir_name = f"updated_codes_attempt_{update_attempt}"
    save_path = os.path.join(state['run_path'], save_dir_name)
    os.makedirs(save_path, exist_ok=True)
    st.write(f"Saving updated files to: `{save_path}`")
    ...
    if state.get("testbench_file") and tb_to_save:
        ...
        file_path = os.path.join(save_path, tb_filename)
        with open(file_path, 'w') as f: f.write(tb_to_save)
        # IMPORTANT: Add the testbench to the list of files for the simulator
        saved_verilog_files.append(os.path.relpath(file_path, state['run_path']))
        st.write(f"  - Saved `{tb_filename}`")
...
def icarus_simulation_agent(state: AgentState) -> Dict[str, Any]:
    ...
    run_path = state['run_path']
    # Use the latest, versioned files for simulation
    verilog_files_to_sim = [os.path.join(run_path, f) for f in state['verilog_files']]
    src_dir = os.path.dirname(verilog_files_to_sim[0])
    output_vvp_file = os.path.join(run_path, "design.vvp")
...
def simulation_verifier_agent(state: AgentState) -> Dict[str, Any]:
    """
    Checks the simulation output for a 'PASSED' or 'SUCCESS' message.
    """
    st.write("---")
    st.write("### 🧐 Agent 6.5: Simulation Verifier")
    st.info("This agent checks the simulation output for a 'PASSED' or 'SUCCESS' message from the self-checking testbench.")
    ...
    simulation_output = state.get("simulation_output", "")
    # Sanitize the string by removing non-ASCII characters before searching.
    # This prevents errors with strange characters in the tool output.
    ascii_output = simulation_output.encode('ascii', 'ignore').decode('ascii')
    
    # Use a case-insensitive regex to find the success keywords
    if re.search(r'\b(PASSED|SUCCESS)\b', ascii_output, re.IGNORECASE):
        st.success("✅ Verification PASSED: Success keyword found in simulation output.")
        return {"simulation_verified": True}
    else:
...
def setup_agent(state: AgentState) -> Dict[str, Any]:
    ...
    if config_or_dict:
        ...
        config = Config.interactive(
            design_name, PDK="gf180mcuC",
            CLOCK_PORT="clk", CLOCK_NET="clk", CLOCK_PERIOD=10.0, # Start with a reasonable clock period
            PRIMARY_GDSII_STREAMOUT_TOOL="klayout",
        )
    st.write("✅ OpenLane configuration loaded.")
    st.info(f"**Design Name for this run: {config['DESIGN_NAME']}**")
...
def synthesis_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Filter out testbenches and non-design files
    synthesizable_files = [f for f in state["verilog_files"] if "_tb" not in f.lower() and "tb." not in f.lower() and "testbench" not in f.lower()]
    st.write("Synthesizing the following files:")
    for f in synthesizable_files:
        st.write(f"- `{f}`")
...
def pin_reduction_corrector_agent(state: AgentState) -> Dict[str, Any]:
    ...
    typically by introducing serialization (SIPO/PISO).
    """
    st.write("---")
    st.write("### 🧠 Agent 27: Pin Reduction Corrector (LLM)")
    st.info("This specialized agent uses an LLM to rewrite the Verilog code to reduce the number of I/O pins.")
...
def pin_reduction_testbench_agent(state: AgentState) -> Dict[str, Any]:
    """
    Rewrites the testbench to work with the new, pin-reduced (likely serial) I/O.
    This is a wrapper that reuses the main testbench corrector agent.
    """
    st.write("---")
    st.write("### 🧠 Agent 28: Pin Reduction Testbench Corrector (LLM)")
    st.info("This agent rewrites the testbench to work with the new, pin-reduced (likely serial) I/O of the design.")
...
# 7. Pin Count Check and Correction Loop
workflow.add_edge("lvs", "pin_counter")
workflow.add_conditional_edges("pin_counter", check_pin_count,
    {"fix_pins": "pin_reduction_corrector", "end": END})
...
workflow.add_edge("pin_reduction_corrector", "pin_reduction_decomposer")
workflow.add_edge("pin_reduction_decomposer", "design_name_updater")
workflow.add_edge("design_name_updater", "pin_reduction_testbench") # Corrected connection
workflow.add_edge("pin_reduction_testbench", "pin_reduction_saver")
...
digraph G {
    ...
    subgraph cluster_correction_pins {
        ...
        pin_reduction_corrector [label="27. Pin Reduction Corrector"];
        pin_reduction_decomposer [label="28. Pin Reduction Decomposer"];
        pin_reduction_testbench [label="29. Pin Reduction Testbench"];
        pin_reduction_saver [label="30. Pin Reduction Saver"];
    }
    ...
    // Loop 3: Pin Reduction (Dark Orange)
    pin_counter -> pin_reduction_corrector [label="Too Many Pins", style=dashed, color="#E65100", fontcolor="#E65100", constraint=false];
    pin_reduction_corrector -> pin_reduction_decomposer -> design_name_updater -> pin_reduction_testbench -> pin_reduction_saver -> icarus_simulation [style=dashed, color="#E65100", arrowhead=normal, label="Re-verify new I/O"];
}
""")
