...
def verilog_corrector_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Determine the primary goal based on feedback
    goal = "The primary goal is to AGGRESSIVELY REDUCE THE DIE AREA of the design. The floorplan is too large. You must simplify the logic to make it smaller."
    optimization_focus = """
...
2.  **Aggressive Bit-width Reduction:** This is the most critical step. Analyze the logic and drastically reduce the bit-width of registers, wires, and parameters. For example, if a 32-bit register only ever holds values up to 100, reduce it to 7 bits (`[6:0]`). You MUST ensure this change is propagated to all connected modules and calculations. Be bold.
3.  **Module Simplification/Removal:** If a module performs a complex function (e.g., sine, cosine) but is used with constant inputs, replace the module instantiation with the pre-calculated result. If a module is instantiated but its functionality is not fully required, simplify or replace it.
4.  **State Machine Optimization:** If there are state machines, consider re-encoding them (e.g., from binary to one-hot) to see if it reduces the logic footprint.
"""

    prompt = f"""
You are an expert Verilog designer and optimizer. Your task is to analyze and rewrite the given Verilog code based on the provided feedback.
...
**OPTIMIZATION & CORRECTION INSTRUCTIONS:**
{optimization_focus}
...
"""
    ...
    for filename, code in code_to_correct.items():
        if "tb" not in filename.lower(): # Exclude testbenches from the prompt
            prompt += f"--- {filename} ---\n{code}\n"
...
def code_decomposer_agent(state: AgentState) -> Dict[str, Any]:
    ...
    RULES:
    1.  Separate each `module` into its own file.
    2.  The filename MUST be the module name with a `.v` extension (e.g., `module_name.v`).
    3.  Your output MUST be a single, valid JSON object where keys are the filenames and values are the complete code content for that file.
    4.  **CRITICAL:** Your final output must be ONLY the JSON object, enclosed in a markdown block. Do NOT add ANY text or explanations before or after the JSON block.
    ...
    **RESPONSE (Valid JSON object only):**
    ...
    try:
        # More robust regex to find JSON within the response, ignoring surrounding text/markdown
        ...
        st.code(response.content, language='text') # Show the problematic response
...
def testbench_corrector_agent(state: AgentState) -> Dict[str, Any]:
    ...
    original_tb_for_diff = state["original_testbench_code"]
    ...
    - Ensure the instantiation of the top-level module in the testbench matches the module's new definition exactly.

    **NEW/UPDATED DESIGN MODULES:**
    ---
    """
    ...
    **ORIGINAL TESTBENCH CODE (`{os.path.basename(state['testbench_file'])}`):**
    ...
    {tb_to_correct}
    ---
    Your task is to provide the updated, complete, and corrected testbench code. Your output must be a single Verilog code block.
    """
    ...
    corrected_tb_code = modified_code_match.group(1).strip()

    st.write("#### Testbench Changes (Diff):")
    diff = difflib.unified_diff(
        original_tb_for_diff.splitlines(keepends=True),
        corrected_tb_code.splitlines(keepends=True),
        fromfile='original_tb.v', tofile='modified_tb.v',
    )
    st.code(''.join(diff), language='diff')

    return {"modified_testbench_code": corrected_tb_code, "original_testbench_code": corrected_tb_code}
...
if uploaded_files:
    ...
    if not verilog_file_names:
        ...
        max_w = st.sidebar.number_input("Max Die Width (mm)", min_value=0.01, value=0.82, step=0.01, format="%.3f")
        max_h = st.sidebar.number_input("Max Die Height (mm)", min_value=0.01, value=0.82, step=0.01, format="%.3f")
