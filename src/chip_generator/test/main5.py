...
def file_saver_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Use the main 'src' directory. Clean it before saving new files.
    save_path = os.path.join(state['run_path'], "src")
...
def icarus_simulation_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Combine design files and the single testbench file for compilation
    ...
    compile_command = ["iverilog", "-g2005-sv", "-o", output_vvp_file, "-I", src_dir] + all_files_for_sim

    try:
        st.write(f"Running compilation: `{' '.join(compile_command)}`")
        # Use a timeout to prevent hanging
        compile_process = subprocess.run(compile_command, capture_output=True, text=True, check=True, timeout=90)
        st.write("✅ Compilation successful.")

        sim_command = ["vvp", output_vvp_file]
        st.write(f"Running simulation: `{' '.join(sim_command)}`")
        sim_process = subprocess.run(sim_command, capture_output=True, text=True, check=True, timeout=90)

        st.write("✅ Simulation process completed.")
        st.text_area("Simulation Output", sim_process.stdout, height=150)
...
def synthesis_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # Filter out testbenches and non-design files from the `src` directory
    src_path = os.path.join(state['run_path'], 'src')
    synthesizable_files = [
    ...
    try:
        with open(report_path) as f:
            ...
            if count_match:
                ...
                st.text_area("DRC Report", content, height=200)
...
def lvs_verifier_agent(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        with open(report_path) as f:
            content = f.read()
            st.text_area("LVS Report", content, height=200)
...
def pin_reduction_corrector_agent(state: AgentState) -> Dict[str, Any]:
    """
    A specialized LLM agent that rewrites Verilog to reduce the number of I/O pins,
    by converting parallel interfaces to serial (shift registers).
    """
    st.write("---")
    st.write("### 🧠 Agent 27: Pin Reduction Corrector (LLM)")
    st.info("This specialized agent uses an LLM to rewrite the Verilog code to reduce the number of I/O pins by implementing a serial interface.")
    ...
    prompt = f"""
    You are an expert Verilog designer specializing in I/O optimization. Your primary, non-negotiable goal is to rewrite the given Verilog HDL to drastically reduce the number of input/output pins.
    The design currently has **{state['pin_count']}** pins, but the maximum allowed is **{state['max_pins']}**.

    **REQUIRED STRATEGY: SERIALIZATION**
    The most effective and required strategy is **serialization**. You MUST convert wide, parallel data inputs and outputs into serial shift registers. This will replace many data pins with just a few control/data pins (like `clk`, `serial_in`, `serial_out`, `load_en`).

    **EXAMPLE OF SERIALIZATION:**
    ---
    // BEFORE: 8-bit parallel input (8 pins)
    module TooManyPins (
        input clk,
        input [7:0] data_in,
        output [7:0] data_out
    ...
    // AFTER: Serial input (clk, serial_in, load_en - 3 pins)
    module PinSaver (
        input clk,
        input load_en,      // Signal to start shifting in data
        input serial_in,    // Single pin for serial data
        output serial_out   // Single pin for serial data out
    );
        reg [7:0] data_in_sr; // Internal shift register
        
        always @(posedge clk) begin
            if (load_en) begin
                data_in_sr <= {{data_in_sr[6:0], serial_in}};
            end
        end
        // ... logic uses data_in_sr ...
    endmodule
    ---

    **RULES:**
    1.  Your output MUST be a single, monolithic block of synthesizable Verilog-2001 code.
    2.  You MUST modify the top-level module's port list to have **fewer than {state['max_pins']}** pins.
    3.  At the top of your code, you MUST include a comment block explaining the serialization strategy you implemented. Example: `// STRATEGY: Converted 32-bit 'data' input into a serial shift register controlled by 'load_en' and 'serial_in'`.
    4.  Do NOT include a testbench. Your output is ONLY the design code.
    5.  Enclose the final Verilog code in a single markdown block.
...
def pin_reduction_testbench_agent(state: AgentState) -> Dict[str, Any]:
    ...
    pin-reduced (serialized) I/O of the design.
    """
    st.write("---")
    ...
    prompt = f"""
    You are an expert Verilog testbench writer. The design under test has been modified to reduce its pin count. It now uses a serial interface instead of a parallel one. You MUST update the testbench to communicate using this new serial protocol.

    **New Pin-Reduced Design Modules:**
    ---
    """
    for filename, code in state['decomposed_files'].items():
        prompt += f"--- {filename} ---\n{code}\n"
    ...
    **CRITICAL INSTRUCTIONS FOR THE NEW TESTBENCH:**
    1.  **Analyze the New Interface:** Examine the new top-level module (`{state['top_level_module']}`) to understand its new serial ports (e.g., `serial_in`, `serial_out`, `load_en`, `clk`).
    2.  **Update Instantiation:** Change the DUT instantiation to match the new module definition.
    3.  **Implement Serial Communication:**
        - To send data to the DUT, you must create a task or loop that drives the `serial_in` pin one bit at a time on each clock edge while a `load_en` (or similar) signal is active.
        - After sending all bits, de-assert the load signal.
    4.  **Wait for Processing:** Add a delay to allow the core logic to compute the result.
    5.  **Receive Serial Data:** Create a similar loop to capture the `serial_out` pin's value on each clock edge to reconstruct the parallel output.
    6.  **Verify Correctness:** Compare the final, reconstructed output with the expected result (as the original testbench did). The core function is the same, only the I/O method has changed.
    7.  **Self-Checking:** The testbench MUST use `$display` to print **EXACTLY** `"Result: PASSED"` or `"Result: FAILED"` and then call `$finish`.

    Provide the updated, complete, and robust self-checking testbench for the **pin-reduced design** in a single Verilog code block.
    """
